...
void pipeline_t::rename2() {
   ...
   //Number of destination valid
   uint64_t numOfDestinationRegisters = 0;

   //Number of branches
   uint64_t numOfBranches = 0;
   ...
   if (REN->stall_reg(numOfDestinationRegisters)) {
	  return;
   }

   if (REN->stall_branch(numOfBranches)) {
	   return;
   }
   ...
   for (i = 0; i < dispatch_width; i++) {
	  ...
	  if (PAY.buf[index].A_valid) {
		 ...
		 // if (index == 156)
			  //printf("Source 1 Physical Mapping : %d \n", PAY.buf[index].A_phys_reg);
	  }
