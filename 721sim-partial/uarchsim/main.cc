...
static void help()
{
  ...
  fprintf(stderr, "  --disambig=<oracle>,<spec>\tEach of <oracle> (oracle memory disambig.) and <spec> (speculative memory disambig.), are 0 or 1\n");
...
static void set_disambig_flags(const char* config) {
   uint64_t omd, smd;
   if (sscanf(config, "%lu,%lu", &omd, &smd) != 2) {
      fprintf(stderr, "Incorrect usage of --disambig=<oracle>,<spec>\n");
      fprintf(stderr, "...where oracle (oracle memory disambig.) and spec (speculative memory disambig.), are each 0 or 1\n");
      exit(-1);
   }
   else {
