...
typedef struct {
   ...
   bool mpt_hit;           // MPT hit or Miss?     /**/
   bool is_addr_pred;      // Is the address predicted for this instruction = Confident
   bool not_confident;
   bool addr_misprediction;
   ...
   unsigned int predicted_addr;         // Predicted address from MPT if MPT hits                             /**/
   unsigned int next_predicted_addr;    // Predicted Address for next occurance of the static instruction     /**/
   ...
   int inst_issue_delay; // #cycles for getting ready and issued
