...
void pipeline_t::register_read(unsigned int lane_number) {
   ...
   if (Execution_Lanes[lane_number].rr.valid) {
	  ...
	  if (PAY.buf[index].C_valid && !(IS_LOAD(PAY.buf[index].flags)) && lat == 1) {
	  ...
	  if (PAY.buf[index].A_valid) {
		  ...
		  if (PAY.buf[index].A_phys_reg == 266) {
			 // printf("Source 1 Value : %d \n", PAY.buf[index].A_value.dw);
		  }
	  }
      ...
      Execution_Lanes[lane_number].ex[0].predicted = Execution_Lanes[lane_number].rr.predicted;      /**/
	  //printf("Register Read predicted   %d \n", Execution_Lanes[lane_number].rr.predicted);
