...
void issue_queue::dispatch(unsigned int index, unsigned long long branch_mask, unsigned int lane_id,
                           ...
                           bool D_valid, bool D_ready, unsigned int D_tag, bool predicted) {    /**/
	unsigned int free;
...
void issue_queue::select_and_issue(unsigned int num_lanes, lane* Execution_Lanes) {
   ...
   for (j = 0; j < size; j++) {
	...
	// inst_issue_delay increment 

	if (!q[i].predicted) {

		proc->PAY.buf[q[i].index].inst_issue_delay++;
		 ...
		 if (issue) {
			 ...
			 if (q[i].predicted) {
				 //printf("Predicted Instruction issued to execute \n");
			 }

			 if (q[i].index == 156) {
				// printf("156th Instruction issued \n");
			 }
				...
				Execution_Lanes[q[i].lane_id].rr.predicted = q[i].predicted;        /**/
